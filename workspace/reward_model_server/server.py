...
def main(cfg) -> None:
    ...
    reward_model_type = RewardModelType(
        cfg.model.get("reward_model_type", "binary_ranking")
    )
    ...
    if torch.distributed.get_rank() == 0:
        infer_callable = RewardModelCallable(
            model_name="reward_model", infer_fn=infer_fn, lock=threading.Lock()
        )
        ...
        model_config = ModelConfig(
            batching=True, max_batch_size=max_batch_size, batcher=dynamic_batcher
        )
