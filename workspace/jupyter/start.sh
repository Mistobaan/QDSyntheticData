...
# the ubuntu user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ]; then
    ...
    # Refit the ubuntu user to the desired user (NB_USER)
    if id ubuntu &> /dev/null; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" ubuntu 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the ubuntu user:"
            _log "- username: ubuntu       -> ${NB_USER}"
            _log "- home dir: /home/ubuntu -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the ubuntu user nor '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the ubuntu home directory to the desired user's home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "ubuntu" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/ubuntu to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            # shellcheck disable=SC2086
            if cp ${CP_OPTS:--a} /home/ubuntu/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/ubuntu to /home/${NB_USER}!"
                _log "Attempting to symlink /home/ubuntu to /home/${NB_USER}..."
                if ln -s /home/ubuntu "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/ubuntu to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
...
# user we started as.
    ...
    ubuntu_UID="$(id -u ubuntu 2>/dev/null)"  # The default UID for the ubuntu user
    ubuntu_GID="$(id -g ubuntu 2>/dev/null)"  # The default GID for the ubuntu user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old ubuntu user to nayvoj ($(id -u ubuntu):$(id -g ubuntu))"
            ...
            sed --expression="s/^ubuntu:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/ubuntu:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "ubuntu" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/ubuntu. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/ubuntu ]]; then
        _log "WARNING: no write access to /home/ubuntu. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
