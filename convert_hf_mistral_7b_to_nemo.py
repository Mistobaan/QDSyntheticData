...
def load_config(mistral_config, tokenizer_path):
    nemo_config = OmegaConf.load(
        os.path.join(os.path.dirname(__file__), "megatron_llama_config.yaml")
    ).model
...
def load_mistral_ckpt(in_dir):
    ...
    added_tokens_path = os.path.join(in_dir, "added_tokens.json")
    if os.path.exists(added_tokens_path):
        with open(added_tokens_path, "r") as fp:
            added_tokens_conf = json.load(fp)
        model_args["vocab_size"] -= len(added_tokens_conf)
